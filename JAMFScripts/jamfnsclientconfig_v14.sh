...
if [ $# -lt 6 ] && [ $idP_mode -eq 0 ]; then
   ...
   echo "Usage: jamfnsclientconfig.sh for email based, <dummy param 1> <dummy param 2>  <currentUsername> <tenant url> <AD domain name> <Rest API token>"
   echo "Usage: jamfnsclientconfig.sh for upn based, <dummy param 1> <dummy param 2>  <currentUsername> <Adonman url>  <Org Key> <upn>"
   ...
   echo "Usage: jamfnsclientconfig.sh for reading email from preference file mode , <dummy param 1> <dummy param 2> <dummy param 3> <tenant url> <Rest API token> <preference_file_name> preference_email"
   echo "Usage: jamfnsclientconfig.sh for reading logged in user using CLI tools, <dummy param 1> <dummy param 2> <dummy param 3> <tenant url> <Rest API token> cli_mode"
...
then
    ...
    echo -n > "${NSINSTPARAM_JSON_FILE}"
     ...
     restAPIToken="$5"
     echo "restAPIToken is $restAPIToken"
    ...
    echo "{\"TenantHostName\":\"$tenantUrl\", \"Email\":\"$emailAddress\", \"RestApiToken\":\"$restAPIToken\"}" > "${NSINSTPARAM_JSON_FILE}"
...
then
    ...
    echo "{\"TenantHostName\":\"$tenantUrl\", \"Email\":\"$emailId\", \"RestApiToken\":\"$restAPIToken\"}" > "${NSINSTPARAM_JSON_FILE}"
